def move(beforedisk,nowlist,beforemove,ans,step,showABC):
    ...
    if beforemove[0] == -1:
        print("step",step,"now is",nowlist)
    else:
        print("step",step,"disk",beforedisk,"from",showABC[beforemove[0]],"move to",showABC[beforemove[1]],"and now is",nowlist)
    if nowlist[2] == ans:
        return
    if beforemove[1] == 0:
    ...
    if len(nowmove) <= 1:
        ...
        if (beforedisk - a) % 2 == 0:
            ...
            if nowlist[nowmove[0][1]] == [] or nowlist[nowmove[0][1]][-1] != beforedisk:
                ...
                nowlist[nowmove[0][0]].remove(a)
                beforemove = nowmove[0]
    move(a,nowlist,beforemove,ans,step+1,showABC)
